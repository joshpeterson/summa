...
class TestArticleParser < Minitest::Test
  ...
  def test_VerifyArticleContrary
    expected_contrary = "On the contrary, It is written (2 Tim. 3:16):"\
                        ...
                        " is no part of philosophical science, which has"\
                        " been built up by human reason. Therefore it"\
                        " is useful that besides philosophical science,"\
  ...
  def test_VerifyArticleAnswer
    expected_answer = "I answer that, It was necessary for man's"\
                      ...
                      " because man is directed to God, as to an end"\
                      " that surpasses the grasp of his reason: \"The"\
                      " eye hath not seen, O God, besides Thee, what"\
                      " things Thou hast prepared for them that wait"\
                      " for Thee\" (Is. 66:4). But the end must first"\
                      " be known by men who are to direct their thoughts"\
                      " and actions to the end. Hence it was necessary"\
                      " for the salvation of man that certain truths"\
                      " which exceed human reason should be made known"\
                      " to him by divine revelation. Even as regards"\
                      " those truths about God which human reason could"\
                      " have discovered, it was necessary that man"\
                      " should be taught by a divine revelation; because"\
                      " the truth about God such as reason could"\
                      " discover, would only be known by a few, and that"\
                      " after a long time, and with the admixture of"\
                      " many errors. Whereas man's whole salvation,"\
                      " which is in God, depends upon the knowledge of"\
  ...
  def test_VerifyFirstObjectionStatement
    expected_statement = "Objection 1: It seems that, besides"\
                          ...
                          " seek to know what is above reason: \"Seek"\
                          " not the things that are too high for thee\""\
                          " (Ecclus. 3:22). But whatever is not above"\
                          " reason is fully treated of in philosophical"\
                          " science. Therefore any other knowledge"\
                          " besides philosophical science is superfluous."
    assert_equal(expected_statement,
  ...
  def test_VerifyFirstObjectionReply
    expected_reply = "Reply to Objection 1: Although those things which"\
                      ...
                      " understanding of man\" (Ecclus. 3:25). And in"\
  ...
  def test_VerifySecondObjectionStatement
    expected_statement = "Objection 2: Further, knowledge can be"\
                          ...
                          " is, is true. But everything that is, is"\
                          " treated of in philosophical science---even"\
                          " God Himself; so that there is a part of"\
                          " philosophy called theology, or the divine"\
                          " science, as Aristotle has proved (Metaph."\
                          " vi). Therefore, besides philosophical"\
  ...
  def test_VerifySecondObjectionReply
    expected_reply = "Reply to Objection 2: Sciences are differentiated"\
                      ...
                      " the physicist both may prove the same"\
                      " conclusion: that the earth, for instance, is"\
                      " round: the astronomer by means of mathematics"\
                      ...
                      " be learned from philosophical science, so far as"\
                      " they can be known by natural reason, may not"\
                      " also be taught us by another science so far as"\
                      " they fall within revelation. Hence theology"\
                      " included in sacred doctrine differs in kind from"\
